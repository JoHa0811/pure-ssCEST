...
#offsets_ppm = np.array([-200,-50,0,50])
seq_defs: dict = {}
...
n_dummies_before_first_offset = 40
n_dummies_between_offsets = 10

# n_sat / readout
n_sat_per_readout = 1
# n_readouts / sat
n_readouts_per_sat = 1
...
filename += f'_{fov * 1000:.0f}mm_{n_spirals_for_traj_calculation}_spirals_{adc_total_samples}k0_{n_slices}slices_{n_readouts_per_slice}interleaves_{n_dummies_between_offsets}dummies_{b1}_{sat_time}'


# create folder for seq and header file
output_path = Path.cwd() / 'SpiralssCEST/output' / filename
...
for m, offset in enumerate(offsets_hz):
    ...
    for repetition in range(n_readouts_per_slice):
        ...
        for slice in ordered_slice_indices:
            ...
            # add global sat pulse and spoiler
            for _ in range(n_sat_per_readout):
                seq.add_block(sat_pulse)
                seq.add_block(post_spoil_x, post_spoil_y, post_spoil_z)
            ...
            for _ in range(n_readouts_per_sat):
                seq.add_block(rf, gz)
    ...
    seq.add_block(
        pp.make_delay(tr_delay),
        pp.make_label(label='REP', type='INC', value=1), # increase offset counter
    )
